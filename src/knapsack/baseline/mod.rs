...
fn simulated_annealing(state: &mut State, rng: &mut Rng, n_rounds: usize, n_iter: usize, alpha: f64) {
    let n = state.ch.num_items;
    let cap = state.ch.max_weight;
...
impl Hparams {
    /// `budget` is capacity as a percent of total item weight (`max_weight * 100 / sum_w`).
    /// Bands: ≤5%, ≤10%, ≤25%, ≤50%, >50% — tighter knapsacks get more SA/ILS; mid/high bands
    /// use full-window VND + medium 2–2 moves for dense QKP; very loose budgets taper cost slightly.
    fn for_size(n: usize, budget: u32) -> Self {
        if n <= 1200 {
            if budget <= 5 {
                ...
                Self {
                    n_random_starts: 4, n_crossover_gen: 10, sa_rounds: 26,
                    sa_iter: 300, n_sa_members: 3, ils_rounds: 280,
                    ils_restart_interval: 12, perturb_base_frac: 4,
                    perturb_max_frac: 5, ils_vnd_level: 1, bounded_2_2_k: 22,
                    n_full_restarts: 1, use_hub_pair: true,
                    use_heavy_polish: true, window_k: n, core_half_dp: 82,
                    sa_alpha: 0.95,
                }
            } else if budget <= 50 {
                Self {
                    n_random_starts: 4, n_crossover_gen: 12, sa_rounds: 28,
                    sa_iter: 300, n_sa_members: 3, ils_rounds: 320,
                    ils_restart_interval: 12, perturb_base_frac: 4,
                    perturb_max_frac: 5, ils_vnd_level: 1, bounded_2_2_k: 50,
                    n_full_restarts: 1, use_hub_pair: true,
                    use_heavy_polish: true, window_k: n, core_half_dp: 110,
                    sa_alpha: 0.97,
                }
            } else {
                Self {
                    n_random_starts: 4, n_crossover_gen: 10, sa_rounds: 22,
                    sa_iter: 300, n_sa_members: 3, ils_rounds: 260,
                    ils_restart_interval: 12, perturb_base_frac: 4,
                    perturb_max_frac: 5, ils_vnd_level: 1, bounded_2_2_k: 60,
                    n_full_restarts: 1, use_hub_pair: true,
                    use_heavy_polish: true, window_k: n, core_half_dp: 120,
                    sa_alpha: 0.97,
                }
            }
        } else if budget <= 5 {
            Self {
                n_random_starts: 4, n_crossover_gen: 10, sa_rounds: 36,
                sa_iter: 280, n_sa_members: 3, ils_rounds: 155,
                ils_restart_interval: 12, perturb_base_frac: 7,
                perturb_max_frac: 5, ils_vnd_level: 1, bounded_2_2_k: 10,
                n_full_restarts: 1, use_hub_pair: true,
                use_heavy_polish: true, window_k: 460, core_half_dp: 58,
                sa_alpha: 0.95,
            }
        } else if budget <= 10 {
            Self {
                n_random_starts: 4, n_crossover_gen: 10, sa_rounds: 30,
                sa_iter: 280, n_sa_members: 3, ils_rounds: 165,
                ils_restart_interval: 12, perturb_base_frac: 7,
                perturb_max_frac: 5, ils_vnd_level: 1, bounded_2_2_k: 10,
                n_full_restarts: 1, use_hub_pair: true,
                use_heavy_polish: true, window_k: 440, core_half_dp: 56,
                sa_alpha: 0.95,
            }
        } else if budget <= 25 {
            Self {
                n_random_starts: 4, n_crossover_gen: 8, sa_rounds: 24,
                sa_iter: 280, n_sa_members: 3, ils_rounds: 175,
                ils_restart_interval: 12, perturb_base_frac: 7,
                perturb_max_frac: 5, ils_vnd_level: 1, bounded_2_2_k: 20,
                n_full_restarts: 1, use_hub_pair: true,
                use_heavy_polish: true, window_k: 410, core_half_dp: 75,
                sa_alpha: 0.95,
            }
        } else if budget <= 50 {
            Self {
                n_random_starts: 4, n_crossover_gen: 8, sa_rounds: 20,
                sa_iter: 280, n_sa_members: 3, ils_rounds: 190,
                ils_restart_interval: 12, perturb_base_frac: 8,
                perturb_max_frac: 5, ils_vnd_level: 1, bounded_2_2_k: 40,
                n_full_restarts: 1, use_hub_pair: true,
                use_heavy_polish: true, window_k: 430, core_half_dp: 90,
                sa_alpha: 0.97,
            }
        } else {
            Self {
                n_random_starts: 4, n_crossover_gen: 6, sa_rounds: 14,
                sa_iter: 280, n_sa_members: 3, ils_rounds: 160,
                ils_restart_interval: 12, perturb_base_frac: 8,
                perturb_max_frac: 5, ils_vnd_level: 1, bounded_2_2_k: 45,
                n_full_restarts: 1, use_hub_pair: false,
                use_heavy_polish: true, window_k: 360, core_half_dp: 90,
                sa_alpha: 0.97,
            }
        }
    }
    ...
    fn from_map(h: &Option<Map<String, Value>>, n: usize, budget: u32) -> Self {
        ...
        if let Some(m) = h {
            ...
            if let Some(v) = m.get("sa_alpha").and_then(|v| v.as_f64()) { p.sa_alpha = v; }
        }
        p
    }
}
...
fn run_one_instance(challenge: &Challenge, hp: &Hparams, rng_offset: usize, save_fn: Option<&dyn Fn(&Solution) -> Result<()>>) -> Solution {
    ...
    if hp.sa_rounds > 0 {
        for pi in 0..hp.n_sa_members.min(population.len()) {
            ...
            simulated_annealing(&mut state, &mut rng, hp.sa_rounds, hp.sa_iter, hp.sa_alpha);
            vnd_dispatch(&mut state, hp);
    ...
    // Save the initial best before ILS so we always have a solution
    if let Some(save) = save_fn {
        let _ = save(&Solution { items: best_sel.clone() });
    }
...
impl Solver {
    pub fn solve(
        ...
        for restart in 0..n_restarts {
            let sol = run_one_instance(challenge, &hp, restart, save_solution);
            let mut val: i64 = 0;
            for &i in &sol.items {
